...
public class Match3 : MonoBehaviour
{
    ...
    List<Point> isConnected(Point p, bool main)
    {
        ...
        foreach (Point dir in directions) //Checking if there is 2 or more same shapes in the directions
        {
            ...
            if (same > 1) // 같은 도형이 1개 보다 많으면 매치
                AddPoints(ref connected, line); // 이 포인트를 connected 리스트에 추가
        }

        for(int i = 0; i < 2; i++) // 가운데 도형에서 왼쪽 오른쪽 체크
        {
            List<Point> line = new List<Point>();

            int same = 0;
            Point[] check = { Point.add(p, directions[i]), Point.add(p, directions[i + 2]) };
            foreach(Point next in check) // 양쪽이 같은 도형인지 체크
            {
        ...
        for(int i = 0; i < 4; i++) // 2x2 체크
        {
            ...
            foreach (Point pnt in check) // 4개가 같은 도형인지 체크
            {
            ...
            if (same > 2) // 같은 도형이 2개 보다 많으면 매치
                AddPoints(ref connected, square); // 이 포인트를 connected 리스트에 추가
        }

        if(main) // 현재 매치와 이어진 다른 매치 체크
        {
    ...
    {
        string seed = "";
        string acceptableChars = "ABCEDEFGHIJKLMNOPQRSTUVWXYZabcedfghijklmnopqrstuvwxyz1234567890!@#$%^&*()";
        for (int i = 0; i < 20; i++)
